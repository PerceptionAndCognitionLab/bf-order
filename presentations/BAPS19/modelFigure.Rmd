...
combiDeltas[combiDeltas$figType == "Model" & combiDeltas$Model == "Positive-effects Model", "Density"] <- as.vector(PosH)
combiDeltas[combiDeltas$figType == "Prediction" & combiDeltas$Model == "Positive-effects Model", "Density"] <- as.vector(PosP)
```
...
combiDeltas[combiDeltas$figType == "Model" & combiDeltas$Model == "Spike-and-Slab Model", "Density"] <- as.vector(MixH)
...
```{r teamAFig, fig.align='center', fig.width= 7, fig.height=3, fig.cap= "Model and predictions for Team A. a. The dashed arrow shows the null model (no effect), the solid line shows the prior distribution for the alternative model of a positive effect of media story on favorability ratings. Higher values represent higher plausibility of the favorability effect before the data have been collected. b. The dashed curve shows the prediction of the no-effect model, the solid curve shows the prediction of the positive-effect model. Predictions take into account sampling noise and are smeared versions of the models themselves. The solid red line represents the observed favorability effect. The data are better predicted by the positive-effect model than by the no-effect model; the Bayes factor favors the positive-effect model 3.61-to-1."}
...
arrowPos <- data.frame(x1 = 0, x2 = 0
                       ...
                       , y2 = max(nrmlz(tTruth(gamma, sd = sd0))) + .02)

lims <- c(0, arrowPos$y2)

model <- rep(c("Null", "Positive", "Negative", "General"), each = length(gamma))
modelDens <- data.frame(model = model, theta = rep(gamma, 4), density = NA, prediction = NA)
...
Neg.m <- makeModelPlot(z = "Density"
                                , dat = subset(combiDeltas
                                               , Model == "Negative-effects Model" & figType == "Model")
                                , ylab = expression(paste(theta["2"]))
                                , xlab = expression(paste(theta["1"]))
                                , breaks = rev(breaks)
                       , last = T)
...
Neg.p <- makeModelPlot(z = "Density"
                                , dat = subset(combiDeltas
                                      , Model == "Negative-effects Model" & figType == "Prediction")
                                , ylab = expression(paste(hat(theta)["2"]))
                           , xlab = expression(paste(hat(theta)["1"]))
                                , breaks = rev(breaks)
                                )
...
```{r combine, fig.asp = .5, fig.width=8}
beautyPlot <- function(uniPlot, multiPlot, title = ""){
  first_col <- plot_grid(uniPlot + theme(plot.margin = unit(c(0., 0, 0, 1.3), "cm")) +
  ...
  second_col <- plot_grid(NULL
          , uniPlot + coord_flip() + theme(plot.margin = unit(c(0, 0, 1.4, 0), "cm")) +
                         geom_hline(yintercept = c(0))
          , ncol = 1
          , rel_heights = c(1, 3)
          )
