...
plotmat(M,name=names,pos=c(1,4,1),curve=0,box.type="round",box.size=.04,box.prop=1,box.col=c('wheat'),arr.length=0,arr.type='triangle')
# box()
mtext(side=3,adj=0,line=-2,"D",cex=1.4)

M=matrix(nrow=length(names),byrow=F,ncol=length(names),data=0)
plotmat(M,name=names,curve=0,box.type="round",box.size=.04,box.prop=1,box.col=c('wheat'),arr.length=0,arr.type='triangle')
# box()
mtext(side=3,adj=0,line=-2,"E",cex=1.4)
